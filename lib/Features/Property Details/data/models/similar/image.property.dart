class ImageSimilarModel {
  int? id;
  String? url;
  int? sortOrder;

  ImageSimilarModel({this.id, this.url, this.sortOrder});

  factory ImageSimilarModel.fromJson(Map<String, dynamic> json) {
    return ImageSimilarModel(
      id: json['id'] as int?,
      url: json['url'] as String?,
      sortOrder: json['sort_order'] as int?,
    );
  }

  Map<String, dynamic>
  () {
    return {'id': id, 'url': url, 'sort_order': sortOrder};
  }
}
